...
class MaskFlownet(nn.Module):
    ...
    def forward(self, im1, im2, raftflow):
        ...
        raftflow_rw = flows[0].shape[2]
        raftflow_rh = flows[0].shape[3]
        raftflow = torch.Tensor(cv2.resize(raftflow[0].cpu().numpy(),(raftflow_rw,raftflow_rh))/20).permute(2,0,1).view(1,2,raftflow_rh,raftflow_rw).to("cuda")
        print("raftflow.size()",raftflow.size())
        rnd = random.random()
